...
uint8_t usbRxDataBuffer[USB_BUF_SIZE];

// USB transmit data buffer
uint8_t usbTxDataBuffer[USB_BUF_SIZE];
...
USB_DEVICE_HID_EVENT_RESPONSE APP_USB_USBDeviceHIDEventHandler(
  ...
  switch (event) {
    case USB_DEVICE_HID_EVENT_REPORT_SENT:
      ...
      if ( reportSent->handle == appData.usbTxTransferHandle ) {
        // Transfer progressed.
    ...
    case USB_DEVICE_HID_EVENT_REPORT_RECEIVED:
      ...
      if(reportReceived->handle == appData.usbRxTransferHandle ) {
        // Transfer progressed.
        appData.hidDataReceived = 0x1;
      }
    ...
    case USB_DEVICE_HID_EVENT_SET_IDLE:
      ...
      appData.hidIdleRate = ((USB_DEVICE_HID_EVENT_DATA_SET_IDLE*)eventData)->duration;
      break;

    case USB_DEVICE_HID_EVENT_GET_IDLE:

      /* Host is requesting for Idle rate. Now send the Idle rate */
      USB_DEVICE_ControlSend(appData.usbDevHandle, & (appData.hidIdleRate),1);
...
void APP_USB_USBDeviceEventHandler(USB_DEVICE_EVENT event, void * eventData, uintptr_t context) {
  ...
  switch(event) {
    ...
    case USB_DEVICE_EVENT_CONFIGURED:
      ...
      appData.usbDeviceConfigured = 0x1;

      /* Save the other details for later use. */
      appData.usbConfigurationValue = ((USB_DEVICE_EVENT_DATA_CONFIGURED*)eventData)->configurationValue;

      /* Register application HID event handler */
      USB_DEVICE_HID_EventHandlerSet(USB_DEVICE_HID_INDEX_0, APP_USB_USBDeviceHIDEventHandler, (uintptr_t)&appData);
